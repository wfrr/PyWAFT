...
class ShoppingCartPage(BasePage):
    ...
    __product_name_locator = (By.CSS_SELECTOR, '#checkout-cart tbody td:nth-of-type(2) a')
    __model_locator = (By.CSS_SELECTOR, '#checkout-cart tbody td:nth-of-type(3)')
    __quantity_locator = (By.CSS_SELECTOR, '#checkout-cart tbody td:nth-of-type(4) input[type=text]')
    ...
    def get_n_entries(self, entry_num: int = 1) -> list[dict[str, str]]:
        ...
        with allure.step(f'Получение {entry_num} записей из корзины'):
            ...
            for name, model, quantity in zip(self.get_all_elements_text(self.__product_name_locator),
                                             self.get_all_elements_text(self.__model_locator),
                                             self.get_all_elements_text(self.__quantity_locator,
                                                                        is_attribute=True)
                                             ):
                entries.append(dict(name=name, model=model, quantity=quantity))
