...
class HomePage(BasePage):
    ...
    _side_menu_btn: Annotated[
        WebElement, By.CSS_SELECTOR, ".v-toolbar__content > button:nth-child(1)"
    ]
    _side_menu_profile_link: Annotated[
        WebElement, By.CSS_SELECTOR, 'nav a[href="/user/profile"]'
    ]
    _shopping_lists_menu_entry: Annotated[
        WebElement, By.CSS_SELECTOR, 'a[href="/shopping-lists"]'
    ]
    ...
    def ensure_side_menu_open(self) -> None:
        ...
        with allure.step("Открытие боковой панели меню"):
            if not self._side_menu_profile_link.is_displayed():
                ...
                WebDriverWait(self.driver, self.timeout).until(
                    ec.element_to_be_clickable(self._side_menu_profile_link)
                )

    def open_shopping_lists(self) -> ShoppingListsPage:
        """Открытие страницы Shopping Lists."""
        with allure.step("Открытие страницы Shopping Lists"):
            self.ensure_side_menu_open()
            WebDriverWait(self.driver, self.timeout).until(
                ec.element_to_be_clickable(self._shopping_lists_menu_entry)
            ).click()
